...
def main(newpage: ft.Page):
    ...
    setting_page = [ft.Column(
        ...
        ,ft.Row(
            [
                ft.Container(width=90),
                ft.Switch(label="监控反馈", label_position='left', scale=1.2,value=fun1,on_change=switch_fun1),
                ft.Container(width=10),
                ft.TextField(label="HA_MQTT账户", width=200,on_submit=input_user,value=read_user)
            ]
        ),ft.Row(
            [
                ft.Container(width=90),
                ft.Switch(label="远程命令", label_position='left', scale=1.2,value=fun2,on_change=switch_fun2),
                ft.Container(width=10),
                ft.TextField(label="HA_MQTT密码", width=200,on_submit=input_pass,value=read_password)
            ]
           
        ),ft.Row(
            [
                ft.Container(width=90),
                ft.Switch(label="画面传输", label_position='left', scale=1.2,value=fun3,on_change=switch_fun3),
                ft.Container(width=10),
                ft.TextField(label="Secret_id", width=200,on_submit=input_id,value=read_secret_id)
            ]
           
        ),ft.Row(
            [
                ft.Container(width=90),
                ft.Switch(label="自动运行", label_position='left', scale=1.2,value=fun4,on_change=switch_fun4,tooltip="运行gui.py时自动运行勾选的服务"),
                ft.Container(width=10),
                ft.TextField(label="监控刷新间隔", width=200,on_submit=input_interval,value=read_interval)
            ]
